...
export async function POST(req: NextRequest) {
  ...
  // 2. Parse body
  let body: { productId?: unknown; templateId?: unknown; reportId?: unknown }
  try {
    body = (await req.json()) as { productId?: unknown; templateId?: unknown; reportId?: unknown }
  } catch {
    return NextResponse.json(
  ...
  const { data: product, error: prodErr } = await db
    .from('products')
    .select('id, name, url, vertical, enrichment, user_id')
    .eq('id', productId)
    .maybeSingle()
  ...
  try {
    ...
    if (insertErr) {
      ...
      console.error('[landing/generate] DB insert failed:', insertErr.message)
      endLog({ templateId, model: result.model, slots: result.filledSlots.length, persisted: false })
      return NextResponse.json({
        ok: true,
        landingId: null,
        landing: result,
    ...
    endLog({ templateId, model: result.model, slots: result.filledSlots.length, landingId: landingRow.id })
    return NextResponse.json({
      ok: true,
