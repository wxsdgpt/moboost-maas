...
async function llmParse(userText: string): Promise<ParseResult> {
  const res = await fetch(`${OPENROUTER_BASE}/chat/completions`, {
    ...
    headers: {
      ...
      'X-Title': 'Moboost AI MAAS - Brief Parser',
    },
    body: JSON.stringify({
      model: PARSE_MODEL,
  ...
  if (!res.ok) throw new Error(`parse LLM failed: ${res.status}`)
  const data = await res.json()
  const content = data.choices?.[0]?.message?.content || '{}'
  const raw = JSON.parse(content) as Partial<ParseResult>
...
export async function POST(req: NextRequest) {
  ...
  if (OPENROUTER_KEY) {
    try {
      ...
      console.warn('[parse] LLM failed, falling back to heuristic:', err)
