...
async function llmClarify(intake: RawIntake, liveRefs?: ParsedReference[]) {
  ...
  const res = await fetch(`${OPENROUTER_BASE}/chat/completions`, {
    ...
    headers: {
      ...
      'X-Title': 'Moboost AI MAAS - Brief Clarifier',
    },
    body: JSON.stringify({
      model: CLARIFY_MODEL,
  ...
  if (!res.ok) throw new Error(`Clarify LLM failed: ${res.status}`)
...
export async function POST(req: NextRequest) {
  ...
  if (intake.urls && intake.urls.length) {
    try {
      ...
      if (out.errors.length) {
        console.warn('[clarify] some refs failed to fetch:', out.errors)
      }
    } catch (err) {
      console.warn('[clarify] ref fetch failed entirely, continuing without:', err)
    }
  }
  ...
  if (OPENROUTER_KEY) {
    try {
      ...
      console.warn('[clarify] LLM call failed, falling back to heuristic:', err)
