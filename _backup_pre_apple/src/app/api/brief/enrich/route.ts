...
每个数组 3-5 条，简洁可执行。riskNotes 包含合规与抓取问题的提示。`
  ...
  const res = await fetch(`${OPENROUTER_BASE}/chat/completions`, {
    ...
    headers: {
      ...
      'X-Title': 'Moboost AI MAAS - Brief Enrichment',
    },
    body: JSON.stringify({
      model: ENRICH_MODEL,
  ...
  if (!res.ok) throw new Error(`Enrich LLM failed: ${res.status}`)
...
export async function POST(req: NextRequest) {
  ...
  if (toFetch.length) {
    try {
      ...
      console.warn('[enrich] fetch failed, continuing with refs from clarify:', err)
    }
  }
  ...
  if (OPENROUTER_KEY) {
    try {
      ...
      console.warn('[enrich] LLM failed, falling back to heuristic:', err)
