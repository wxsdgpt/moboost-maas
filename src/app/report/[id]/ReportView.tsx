...
export default function ReportView({ report }: Props) {
  ...
  return (
    <div style={{ fontFamily: '-apple-system, "SF Pro Display", "SF Pro Text", "Helvetica Neue", Arial, sans-serif' }}>
            ...
            <div style={{ maxWidth: 980, margin: '0 auto', padding: '0 24px' }}>
              <div style={{ display: 'flex', justifyContent: 'center', gap: 16 }}>
                ...
                <button
                  disabled={navigating}
                  onClick={async () => {
                    const target = `/brief/execute?productId=${report.product_id}&reportId=${report.id}`
                    setNavError(null)
                    setNavigating(true)

                    try {
                      // Pre-flight: verify session is still valid on the server
                      const check = await fetch('/api/me', {
                        credentials: 'same-origin',
                        cache: 'no-store',
                        redirect: 'manual', // don't follow redirects — detect auth failure
                      })

                      // redirect: 'manual' returns status 0 (opaqueredirect) for redirects
                      if (check.type === 'opaqueredirect' || check.status === 0 || check.status === 401) {
                        console.warn('[ReportView] session expired, redirecting to sign-in')
                        // Session expired — redirect to sign-in with return URL
                        window.location.href = `/sign-in?redirect_url=${encodeURIComponent(target)}`
                        return
                      }

                      // Also verify the response is actually JSON from our API
                      const ct = check.headers.get('content-type') || ''
                      if (!ct.includes('application/json')) {
                        console.warn('[ReportView] unexpected response type:', ct)
                      ...
                      if (!data.ok) {
                        console.warn('[ReportView] /api/me returned not-ok:', data)
                        window.location.href = `/sign-in?redirect_url=${encodeURIComponent(target)}`
                        return
                      }

                      // Auth valid — proceed with client-side navigation
                      router.push(target)
                    } catch (err) {
                      console.error('[ReportView] pre-flight check error:', err)
                      // Network error — try direct navigation anyway
                      window.location.href = target
                    } finally {
                ...
                >
                  ...
                  {navigating ? 'Loading...' : 'Execute All — Generate 3 Sets'}
                </button>
              </div>
              <p style={{
      ...
      {/* ════ GENERATED ARTIFACTS ════
          Unlock-gated: a paid/full report shows everything generated for it,
          including re-runs (reverse-chronological). Per-row regenerate uses
          the user's custom prompt and stacks new history rows. */}
      {(unlocked || !isLite) && (
        <ReportArtifacts reportId={report.id} />
      )}
