...
export async function POST(req: NextRequest) {
  ...
  if (existing.data?.onboarded_at) {
    ...
    // Ensure cookie is set even for repeat submissions
    alreadyRes.cookies.set('moboost:onboarded', '1', {
      httpOnly: true,
  ...
  // 2b. Auto-create a default project for this product so every artifact
  //     generated from now on (reports, landing pages, assets) has a
  //     stable home in /project. Without this, generated content orphans.
  const projectInsert = await db
    ...
    .insert({
      ...
      source: 'onboarding',
      metadata: { auto_created: true, productUrl },
    })
    .select('id')
    .single()

  if (projectInsert.error) {
    // Non-fatal — onboarding still succeeds, but log it. The user will
    // see orphaned content until backfill runs.
    console.error('[onboarding] default project insert failed:', projectInsert.error.message)
  }
  ...
  void kickoffEnrichment(productId).catch((err) => {
  ...
  // Set onboarding cookie so middleware skips the DB check on future requests
  response.cookies.set('moboost:onboarded', '1', {
    httpOnly: true,
    secure: process.env.NODE_ENV === 'production',
    sameSite: 'lax',
...
async function kickoffEnrichment(productId: string): Promise<void> {
  ...
  const { error: statusErr } = await db
    ...
    .eq('id', productId)
  if (statusErr) console.error('[onboarding] enrichment status update failed:', statusErr.message)
  ...
  try {
    ...
    const { error: enrichErr } = await db
      ...
      .eq('id', productId)
    if (enrichErr) console.error('[onboarding] enrichment data update failed:', enrichErr.message)
  } catch (err) {
    const msg =
    ...
    const { error: failErr } = await db
      ...
      .eq('id', productId)
    if (failErr) console.error('[onboarding] enrichment failure update failed:', failErr.message)
  }
}
