...
export async function POST(req: NextRequest): Promise<NextResponse<GenerateReportResponse>> {
  ...
  // 2. Parse body
  let body: { productId?: unknown; kind?: unknown; projectId?: unknown }
  try {
    body = (await req.json()) as { productId?: unknown; kind?: unknown; projectId?: unknown }
  } catch {
    return NextResponse.json(
  ...
  try {
    const { reportId, report, projectId } = await generateReport(
      user.id,
      productId,
      kind,
      projectIdOverride,
    ...
    endLog({ reportId, kind, projectId, sections: report?.sections?.length })
    logEvent('credit_committed', user.id, { amount: cost, kind, projectId })
    return NextResponse.json({ ok: true, reportId, report, projectId })
  } catch (err) {
