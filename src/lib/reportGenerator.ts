...
export async function generateReport(
...
): Promise<{ reportId: string; report: ReportOutput; projectId: string | null }> {
  ...
  // 1b. Resolve target project: explicit override > newest project for this
  //     product > newest project for the user. Auto-create one tied to the
  //     product if none exists, so artifacts always have a home.
  const projectId = await resolveProjectId(db, userId, productId, projectIdOverride, product.name, 'report_generation')
  ...
  return { reportId: reportRow.id, report, projectId }
}

// Project resolution moved to src/lib/projectResolver.ts (shared with landing/asset generators).
